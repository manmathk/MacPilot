...
final class CommandHistoryStore: ObservableObject {
    ...
    func record(query: String, result: String) {
        ...
        entries.insert(.init(id: UUID(), query: clean, result: result, date: Date()), at: 0)
    ...
    private func load() {
        guard let data = UserDefaults.standard.data(forKey: key), let decoded = try? JSONDecoder().decode([CommandHistoryEntry].self, from: data) else { return }
        entries = decoded
    }
...
final class StorageIntelligence: ObservableObject {
    ...
    func scan() {
        ...
        DispatchQueue.global(qos: .utility).async {
            let values: [StorageItem] = roots.compactMap { path, category in
    ...
    private nonisolated static func directorySize(_ url: URL) -> Int64 {
        ...
        for case let item as URL in enumerator {
            ...
            if let values = try? item.resourceValues(forKeys: keys), values.isRegularFile == true { total += Int64(values.fileSize ?? 0) }
        }
        return total
    }
}

// MARK: - Finder power tools
...
enum FinderPowerService {
    ...
    static func createFile(named name: String) -> String {
        ...
        let script = "tell application \"Finder\" to make new file at (target of front window) with properties {name:\"\(safe)\"}"
        return run(script).map { "Created \($0)" } ?? "Couldn't create file in Finder"
    }

    static func createFolder(named name: String) -> String {
        let safe = name.replacingOccurrences(of: "\"", with: "\\\"")
        let script = "tell application \"Finder\" to make new folder at (target of front window) with properties {name:\"\(safe)\"}"
        return run(script).map { "Created \($0)" } ?? "Couldn't create folder in Finder"
    }

    static func duplicateSelection() -> String {
        let script = "tell application \"Finder\" to if (count of selection) is 0 then return \"No Finder selection\" else duplicate selection to (target of front window)"
        return run(script) ?? "Couldn't duplicate Finder selection"
    }

    static func renameSelection(to name: String) -> String {
        let safe = name.replacingOccurrences(of: "\"", with: "\\\"")
        let script = "tell application \"Finder\" to if (count of selection) is 0 then return \"No Finder selection\" else set name of item 1 of selection to \"\(safe)\""
        return run(script) ?? "Couldn't rename Finder selection"
    }
}
...
final class AutomationEngine: ObservableObject {
    ...
    private func poll() {
        ...
        let launched = current.subtracting(knownApps)
        let terminated = knownApps.subtracting(current)
        ...
        if NSScreen.screens.count != screenCount { screenCount = NSScreen.screens.count; TimelineStore.shared.add(.displayChange, title: "Display configuration changed", detail: "\(screenCount) screen(s) available") }
        knownApps = current
    }
